...
def exit_handler():
    ...
    '''
    print('[INFO] [UAVSpawner]: Exit requested')

    if len(glob_running_processes) > 0:
        print(f'[INFO] [UAVSpawner]: Shutting down {len(glob_running_processes)} subprocesses')

        num_zombies = 0
        for p in glob_running_processes:
            try:
                p.shutdown()
                print(f'[INFO] [UAVSpawner]: Process {p.run_id} shutdown')
            except:
                num_zombies += 1

        if num_zombies > 0:
            print(f'\033[91m[ERROR] [UAVSpawner]: Could not stop {num_zombies} subprocesses\033[91m')
            exit(1)

    print('[INFO] [UAVSpawner]: Exited gracefully')
    exit(0)
# #}
...
class UAVSpawner:
    ...
    def __init__(self, verbose=False):
        ...
        rospy.init_node('uav_spawner', anonymous=True)

        resource_paths = []
        self.rospack = rospkg.RosPack()
        resource_paths.append(os.path.join(self.rospack.get_path('ctz_uav_simulation'), 'models'))
        ...
        try:
            ...
            rospy.logerr(f'[UAVSpawner]: Could not load required param {err}')
        ...
        for elem in extra_resource_paths:
            ...
            rospy.loginfo(f'[UAVSpawner]: Adding extra resources from {rpath}')
        ...
        gazebo_simulation_path = self.rospack.get_path('ctz_uav_simulation')
        ...
        try:
            ...
            rospy.logerr(f'[UAVSpawner]: {err}')
            raise rospy.ROSInterruptException

        rospy.loginfo(f'[UAVSpawner]: ------------------------')
        rospy.loginfo(f'[UAVSpawner]: Jinja templates loaded:')
        for name in self.jinja_templates.keys():
            rospy.loginfo(f'[UAVSpawner]: "{name}" from "{self.jinja_templates[name].jinja_template.filename}"')
        rospy.loginfo(f'[UAVSpawner]: ------------------------')
    ...
    def get_all_templates(self):
        ...
        for i, full_name in enumerate(template_names):
            rospy.loginfo(f'[UAVSpawner]: \t({i+1}/{len(template_names)}): {full_name}')
                ...
                for elem in node.body:
                    ...
                    if isinstance(elem, jinja2.nodes.Assign) and elem.target.name == 'spawner_default_args':
                        if isinstance(elem.node, jinja2.nodes.Const):
                            ...
                            rospy.logwarn(f'[UAVSpawner]: Unsupported param type "{type(elem.node)}" in template {template.filename}')
    ...
    def build_template_database(self):
        ...
        rospy.loginfo('[UAVSpawner]: Loading all templates')
        ...
        rospy.loginfo('[UAVSpawner]: Reindexing imported templates')
        ...
        rospy.loginfo('[UAVSpawner]: Adding available components from dependencies')
        ...
        rospy.loginfo('[UAVSpawner]: Pruning components to only include callables')
        callable_components = {}
        for name, template in all_templates:
        ...
        rospy.loginfo('[UAVSpawner]: Template database built')
    ...
    def render(self, spawner_args):
        ...
        try:
            ...
            rospy.logerr(f'[UAVSpawner]: Cannot render template, model not specified')
            return

        try:
            template_wrapper = self.jinja_templates[model_name]
        except KeyError:
            rospy.logerr(f'[UAVSpawner]: Cannot render model "{model_name}". Template not found!')
            return

        rospy.loginfo(f'[UAVSpawner]: Rendering model "{model_name}" using template {template_wrapper.jinja_template.filename}')
        ...
        try:
            ...
            rospy.logerr(f'[UAVSpawner]: XML error: "{e}"')
            fd, filepath = tempfile.mkstemp(prefix='uav_spawner_' + datetime.datetime.now().strftime("%Y_%m_%d__%H_%M_%S_"), suffix='_DUMP_' + str(model_name) + '.sdf')
            with os.fdopen(fd, 'w') as output_file:
                output_file.write(rendered_template)
                rospy.loginfo(f'[UAVSpawner]: Malformed XML for model {model_name} dumped to {filepath}')
            return
    ...
    def parse_user_input(self, input_str):
        ...
        for ID in input_dict['ids']:
            if not isinstance(ID, int):
                if ID in self.jinja_templates.keys() and input_dict['model'] is None:
                    rospy.loginfo(f'[UAVSpawner]: Using {ID} as model template')
                    input_dict['model'] = ID
                else:
                    rospy.logwarn(f'[UAVSpawner]: Ignored ID {ID}: Not an integer')
                continue
            if ID < 0 or ID > 255:
                rospy.logwarn(f'[UAVSpawner]: Ignored ID {ID}: Must be in range(0, 256)')
                continue
            if ID in self.assigned_ids:
                rospy.logwarn(f'[UAVSpawner]: Ignored ID {ID}: Already assigned')
                continue
            valid_ids.append(ID)
        ...
        if len(valid_ids) > 0:
            rospy.loginfo(f'[UAVSpawner]: Valid robot IDs: {valid_ids}')
        ...
        if 'pos' in input_dict.keys():
            try:
                ...
                rospy.logerr(f'[UAVSpawner]: While parsing args for "--pos": {err}')
                rospy.logwarn(f'[UAVSpawner]: Assigning random spawn poses instead')
            ...
            try:
                ...
                rospy.logerr(f'[UAVSpawner]: While parsing args for "--pos-file": {err}')
                rospy.logwarn(f'[UAVSpawner]: Assigning random spawn poses instead')
                input_dict['spawn_poses'] = self.get_randomized_spawn_poses(input_dict['ids'])
            finally:
    ...
    def get_model_help_text(self, model_name):
        ...
        Component name
            ...
            Default args:
        '''
        rospy.loginfo(f'[UAVSpawner]: Getting help for model {model_name}')
        try:
            template_wrapper = self.jinja_templates[model_name]
            response = f'[UAVSpawner]: Components used in template "{template_wrapper.jinja_template.filename}":\n'
        except ValueError:
    ...
    def get_spawner_help_text(self):
        ...
        rospy.loginfo(f'[UAVSpawner]: Getting generic spawner help')
    ...
    def callback_spawn(self, req):
        ...
        rospy.loginfo(f'[UAVSpawner]: Spawn called with args "{req.value}"')
        ...
        try:
            ...
            rospy.logwarn(f'[UAVSpawner]: While parsing user input: {e}')
        ...
        rospy.loginfo(f'[UAVSpawner]: Spawner params assigned "{params_dict}"')

        # #{ check gazebo running
        try:
            rospy.loginfo('[UAVSpawner]: Waiting for /gazebo/model_states')
        ...
        rospy.loginfo('[UAVSpawner]: Adding vehicles to a spawn queue')
    ...
    def callback_action_timer(self, timer):
        ...
        if len(self.vehicle_queue) > 0:
            ...
            rospy.loginfo(f'[UAVSpawner]: Vehicle {robot_params["name"]} successfully spawned')
    ...
    def assign_free_id(self):
        ...
        for i in range(0, 256): # 255 is a hard limit of px4 sitl
            if i not in self.assigned_ids:
                rospy.loginfo(f'[UAVSpawner]: Assigned free ID "{i}" to a robot')
                return i
    ...
    def get_spawn_poses_from_file(self, filename, ids):
        ...
        rospy.loginfo(f'[UAVSpawner]: Loading spawn poses from file "{filename}"')
        ...
        rospy.loginfo(f'[UAVSpawner]: Spawn poses returned: {spawn_poses}')
        return spawn_poses
    # #}
    ...
    def get_spawn_poses_from_args(self, pos_args, ids):
        ...
        if len(ids) > 1:
            rospy.logwarn(f'[UAVSpawner]: Spawning more than one vehicle with "--pos". Each additional vehicle will be offset by {self.model_spacing} meters in X')
    ...
    def get_jinja_params_for_one_robot(self, params_dict, index, ID):
        ...
        if 'enable_mavlink_gcs' in params_dict.keys():
            ...
            rospy.loginfo(f'[UAVSpawner]: Publishing extra mavlink messages on UDP port {robot_params["mavlink_gcs_udp_port_remote"]}')
    ...
    def launch_px4_firmware(self, robot_params):
        name = robot_params['name']
        rospy.loginfo(f'[UAVSpawner]: Launching PX4 firmware for {name}')
        ...
        if not os.path.exists(romfs_path) or not os.path.isdir(romfs_path):
            rospy.logerr(f'[UAVSpawner]: Could not start PX4 firmware for {name}. ROMFS folder not found')
        ...
        try:
            ...
            rospy.logerr(f'[UAVSpawner]: Could not start PX4 firmware for {name}. Node failed to launch')
            raise CouldNotLaunch('PX4 failed to launch')

        rospy.loginfo(f'[UAVSpawner]: PX4 firmware for {name} launched')
    ...
    def launch_mavros(self, robot_params):
        name = robot_params['name']
        rospy.loginfo(f'[UAVSpawner]: Launching mavros for {name}')
        ...
        try:
            ...
            rospy.logerr(f'[UAVSpawner]: Could not start mavros for {name}. Node failed to launch')
            raise CouldNotLaunch('Mavros failed to launch')

        rospy.loginfo(f'[UAVSpawner]: Mavros for {name} launched')
    ...
    def spawn_gazebo_model(self, robot_params):
        ...
        if self.save_sdf_files:
            fd, filepath = tempfile.mkstemp(prefix='uav_spawner_' + datetime.datetime.now().strftime("%Y_%m_%d__%H_%M_%S_"), suffix='_' + str(robot_params['model']) + '_' + str(name) + '.sdf')
            with os.fdopen(fd, 'w') as output_file:
                output_file.write(sdf_content)
                rospy.loginfo(f'[UAVSpawner]: Model for {name} written to {filepath}')
        ...
        rospy.loginfo(f'[UAVSpawner]: Spawning gazebo model for {name}')
        try:
            response = self.gazebo_spawn_proxy(name, sdf_content, name, spawn_pose, "")
            if not response.success:
                rospy.logerr(f'UAVSpawner]: While calling {self.gazebo_spawn_proxy.resolved_name}: {response.status_message}')
                raise CouldNotLaunch(response.status_message)
        except rospy.service.ServiceException as e:
                rospy.logerr(f'[UAVSpawner]: While calling {self.gazebo_spawn_proxy.resolved_name}: {e}')
    ...
    def delete_gazebo_model(self, name):
        rospy.loginfo(f'[UAVSpawner]: Deleting gazebo model {name}')
        try:
            response = self.gazebo_delete_proxy(name)
            if not response.success:
                rospy.logerr(f'UAVSpawner]: While calling {self.gazebo_delete_proxy.resolved_name}: {response.status_message}')
                return
        except rospy.service.ServiceException as e:
                rospy.logerr(f'[UAVSpawner]: While calling {self.gazebo_delete_proxy.resolved_name}: {e}')
                return

        rospy.loginfo(f'[UAVSpawner]: Model {name} deleted')
    # #}

if __name__ == '__main__':

    print('[INFO] [UAVSpawner]: Starting')
